...
public class AudioManager : SingletonMonoBehaviour<AudioManager>
{
    ...
    private AudioSource[] _seSourceList = new AudioSource[SE_SOURCE_NUM];
    private const int SE_SOURCE_NUM = 8;
    ...
    private new void Awake()
    {
        ...
        for (int i = 0; i < audioSourceArray.Length; i++)
        {
            ...
            {
                _seSourceList[i - 1] = audioSourceArray[i];
    ...
    public void PlaySE(string seName, float delay = 0.0f)
    {
        ...
        //_nextSEName = seName;
        StartCoroutine(DelayPlaySE(_seDic[seName] as AudioClip, delay));
    }

    private IEnumerator DelayPlaySE(AudioClip audio, float delay)
    {
        if (delay > 0f) yield return new WaitForSeconds(delay);
        for (int i = 0; i < _seSourceList.Length; i++)
        {
            if (!_seSourceList[i].isPlaying)
            {
                _seSourceList[i].PlayOneShot(audio);
                yield break;
            }
        }
    }
    ...
    public void ChangeVolume(float BGMVolume, float SEVolume)
    {
        ...
        for (int i = 0; i < _seSourceList.Length; i++)
        {
            _seSourceList[i].volume = SEVolume;
        }
